...
class Solution {
  public:
    int minWastedSpace(vector<int> &V, vector<vector<int>> &boxes) {
        sort(V.begin(), V.end());
        vector<ll> P;
        for (int i : V) {
            P.push_back(i);
            ...
            if (i.back() >= V.back()) {
                ...
                for (int j : i) {
                    ...
                    if (j < V[now])
                        continue;
                    debug(now);
                    int pos = upper_bound(V.begin() + now, V.end(), j) - V.begin() - 1;
                    sum += ll(j) * (pos - now + 1);
                    sum -= P[pos];
                    if (now)
                        sum += P[now - 1];
...
int32_t main() {
    ...
    cout.precision(10);
    cout << fixed;
    vector<int> a{7, 6, 5, 3, 4};
    vector<vector<int>> b{{2, 7}, {6}, {10, 5}};
    Solution s;
    auto res = s.minWastedSpace(a, b);
    debug(res);
    return 0;
}
#endif