...
class Solution {
  public:
    int boxDelivering(vector<vector<int>> &boxes, int portsCount, int maxBoxes, int maxWeight) {
        boxes.insert(boxes.begin(), vector<int>{});
        dfn[1] = 1;
        xs[0] = 0;
        xs[1] = boxes[1][1];
        for (int i = 2; i < (int)boxes.size(); i++) {
            dfn[i] = dfn[i - 1];
            xs[i] = xs[i - 1] + boxes[i][1];
            if (boxes[i][0] != boxes[i - 1][0])
                dfn[i]++;
        }
        Seg<N> seg;
        for (int i = 1; i < (int)boxes.size(); i++) {
            seg.upd(i, dp[i - 1] - dfn[i]);
            int pos = lower_bound(xs, xs + boxes.size(), xs[i] - maxWeight) - xs + 1;
            pos = max(pos, i - maxBoxes + 1);
            int mx = seg.query(pos, i);
            dp[i] = mx + 2 + dfn[i];
            // for (int j = i; j >= pos; j--) {
            //     dp[i] = min(dp[i], dp[j - 1] + 2 + dfn[i] - dfn[j]);
            // }
        }
...
int32_t main() {
    ...
    cout.precision(10);
    cout << fixed;
    vector<vector<int>> data{{1, 1}, {2, 1}, {1, 1}};
    Solution s;
    auto res = s.boxDelivering(data, 2, 3, 3);
    debug(res);
    return 0;
}
#endif