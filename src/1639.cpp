...
class Solution {
  public:
    int numWays(vector<string> &words, string target) {
        const int sz = words.front().size();
        target = "$" + target;
        vector<Int> dp1(target.size());
        dp1[0] = 1;
        for (int i = 0; i < sz; i++) {
            vector<int> cnt(26);
            for (auto &j : words)
                cnt[j[i] - 'a']++;
            for (int j = (int)target.size() - 1; j >= 1; j--) {
                dp1[j] += dp1[j - 1] * cnt[target[j] - 'a'];
            }
        }
        return dp1[target.size() - 1].n;
    }
};
...
int32_t main() {
    ...
    cout.precision(10);
    cout << fixed;
    vector<string> data{"acca", "bbbb", "caca"};
    Solution s;
    string sb = "aba";
    auto res = s.numWays(data, sb);
    debug(res);
    return 0;
}
#endif