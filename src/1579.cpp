...
class Solution {
  public:
    int maxNumEdgesToRemove(int n, vector<vector<int>> &edges) {
        ...
        int ans = 0;
        for (auto [a, b] : V[3]) {
            if (st1.seek(a) != st1.seek(b) || (st2.seek(a) != st2.seek(b))) {
                st1.merge(a, b);
                st2.merge(a, b);
                ans++;
            }
        ...
        for (auto [a, b] : V[2]) {
            if (st2.seek(a) != st2.seek(b)) {
                st2.merge(a, b);
                ans++;
            }
        }
        for (auto [a, b] : V[1]) {
            if (st1.seek(a) != st1.seek(b)) {
                st1.merge(a, b);
                ans++;
            }
        }
        for (int i = 1; i <= n; i++) {
            if (st1.seek(i) != st1.seek(1)) {
                return -1;
            }
        }
        for (int i = 1; i <= n; i++) {
            if (st2.seek(i) != st2.seek(1)) {
                return -1;
            }
        }
