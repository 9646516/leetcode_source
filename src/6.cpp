  ...
  public:
    string convert(const string &s, int numRows) {
        ...
        for (int i = 0; i < (int)s.size(); i += n) {
            int left = min((int)s.size() - i, n);
            solve(V, s.data() + i, left);
        }
        string ret;
        ret.reserve(s.size());
        for (auto &i : V) {
    ...
    void solve(vector<vector<char>> &V, const char *s, int n) {
        for (int i = 0; i < (int)V.size() && n; i++) {
            V[i].push_back(s[i]);
            n--;
        }
        for (int i = V.size() - 2; i >= 0 && n; i--) {
            V[i].push_back(s[V.size() + (V.size() - 2 - i + 1) - 1]);
            n--;
        }
    }
};
