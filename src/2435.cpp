...
class Solution {
  public:
    int numberOfPaths(vector<vector<int>> &grid, int mod) {
        const int N = grid.size();
        const int M = grid.front().size();
        vector<vector<vector<Int>>> dp(N, vector<vector<Int>>(M, vector<Int>(mod)));
        dp[0][0][grid[0][0] % mod] = 1;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                for (int k = 0; k < mod; k++) {
                    if (i + 1 < N) {
                        dp[i + 1][j][(k + grid[i + 1][j]) % mod] += dp[i][j][k];
                    }
                    if (j + 1 < M) {
                        dp[i][j + 1][(k + grid[i][j + 1]) % mod] += dp[i][j][k];
                    }
                }
            }
        }
        return dp[N - 1][M - 1][0].n;
    }
};
#ifdef RINNE
int32_t main() {
